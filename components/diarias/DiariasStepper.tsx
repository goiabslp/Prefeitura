        ...
        <div className="w-full py-4">
            <div className="flex items-center justify-between w-full relative">
                {steps.map((step, index) => {
                    ...
                    if (status === 'completed') {
                        circleClass = 'bg-indigo-500 border-indigo-500 text-white';
                        ...
                        <React.Fragment key={step.id}>
                            ...
                            {!isLast && (
                                <div className="flex-1 h-1 mx-2 rounded-full overflow-hidden bg-slate-100 relative -z-10">
                                    <div
                                        className={`h-full transition-all duration-500 ${status === 'completed' ? 'bg-indigo-500' :
                                                currentStep > step.id ? 'bg-indigo-500' : 'bg-transparent'
                                            }`}
                                    />
                                </div>
                            )}
